...
naniar::miss_summary(outcome_vars_data)["miss_var_summary"] %>% unnest(cols = c(miss_var_summary))
...
sci <- sci %>% 
  ...
  mutate(age_cat = cut(age, breaks = c(0, 30, 45, 60, 75, Inf), labels = c("16-30", "31-45", "46-60","61-75", "75+")),
         ...
         hc_inpatient_num_cat = cut(hc_inpatient_num, breaks = c(-1, 1, 2, 4, Inf), labels = c("0", "1", "2-4", "5+")),
         hc_ambulant_num_cat = cut(hc_ambulant_num, breaks = c(-1, 1, 2, 4, Inf), labels = c("0", "1", "2-4", "5+")),
         hc_inpatient_days_cat = cut(hc_inpatient_days, breaks = c(-1, 1, 5, 20, Inf), labels = c("0", "1-5", "6-20", "21+")),
...
select(sci, hc_ambulant, hc_ambulant_num, hc_ambulant_num_cat) %>% 
  filter(hc_ambulant == 0) %>% 
  mutate_all(~as.integer(as.character(.))) %>% 
  summarize_all(sum)

select(sci, id_swisci, hc_ambulant, hc_ambulant_num, hc_ambulant_num_cat) %>% 
  filter(hc_ambulant == 1) %>% 
  filter(hc_ambulant_num == 0) %>% 
  pull(id_swisci) %>% 
  saveRDS(file.path("workspace", "hc_ambulant_num.Rdata"))


sci %>% 
  select(hc_inpatient_num, hc_inpatient_days_cat)

sci %>% pull(hc_inpatient_num) %>% table(useNA = "always") %>% prop.table()

setdiff(used_vars, names(sci))

used_vars_cat <- used_vars[used_vars != "all"]
used_vars_num <- used_vars_cat %>% str_subset("_cat") %>% str_remove("_cat")

sci <- select(sci, tp, used_vars_cat, used_vars_num)
...
naniar::miss_summary(sci)["miss_var_summary"] %>% unnest(cols = c(miss_var_summary))
...
categ_data <- select(sci, tp, used_vars_cat)

bt <- get_bt_discrete_vars(categ_data, .tp = "ts2", vars_to_remove_missings = used_vars_cat)
