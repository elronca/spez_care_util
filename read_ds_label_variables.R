...
# Make variables matchable by select function

# Load whole dataset

load_swisci_data <- function(path_ds, tp, all_vars, num_vars, cat_vars, codebook) {
  
  # Make variables one string
  
  all_vars <- str_c("^", all_vars, collapse = "|")
  num_vars <- str_c("^", num_vars, collapse = "|")
  cat_vars <- str_c("^", cat_vars, collapse = "|")
  ...
  my_file <- read_csv2(path_ds, col_types = cols(.default = "c")) %>% 
    ...
    select(matches(all_vars)) %>% 
    mutate_at(vars(matches(num_vars)), as.numeric)
  ...
  cat_vars_sep <- str_split(cat_vars, "\\|") %>% pluck(1)
  
  ds_cat_vars <- map_dfc(cat_vars_sep, recode_variables, data = my_file, codebook = codebook) %>% 
    set_names(str_remove(cat_vars_sep, "\\^")) %>% 
    add_column(id_swisci = my_file$id_swisci, .before = 1) %>% 
    mutate_all(str_to_lower)
  
  ds_num_vars <- select(my_file, id_swisci, matches(num_vars))
...
if(FALSE) { 
  ...
  select(data_17_prob, ts2_hc_practitioner, ts2_hc_practitioner_1, ts2_hc_practitioner_num) %>% 
